...
void check() {
    ...
    for(int i=0;i<HASH_SIZE;i++){
    	...
    	while(tmp!=NULL){
    		if(tmp->type->kind == ENUM_FUNC && tmp->type->func->hasDefined == 0){
    ...
    if (strcmp(root->children[0]->name, "Exp") == 0) {
        ...
        //(need fill)
            ...
            if(strcmp(left->children[0]->name, "ID") != 0){
            	if(left->children[0]->childNum>0){
            		Type tmp = Exp(left->children[0]);
            		if(tmp==NULL)
            			return NULL;
            		if(tmp->kind==ENUM_ARRAY && strcmp(left->children[1]->name, "LB")==0){}
            		else if(tmp->kind==ENUM_STRUCT && strcmp(left->children[1]->name, "DOT")==0){}
            		else{
